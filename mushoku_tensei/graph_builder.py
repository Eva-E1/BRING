...
LAYER_BY_TYPE = {
    ...
    "Ability": LAYER_RULE,
    "Skill": LAYER_RULE,
    "MagicSystem": LAYER_RULE,
    "WorldRule": LAYER_RULE,
    "CalendarEra": LAYER_RULE,
    "HistoricalEvent": LAYER_STORY,
    "Arc": LAYER_STORY,
    "NarrativeArc": LAYER_STORY,
    "TimelineEvent": LAYER_STORY,
    "CharacterAge": LAYER_STORY,
    "Memory": LAYER_STORY,
    "Job": LAYER_FACT,
    "Emotion": LAYER_FACT,
...
def build_layered_graph(
    ...
    for idx, segment in enumerate(merged_data):
        entities = deepcopy(segment.get("entities", []))
        edges = deepcopy(segment.get("edges", []))
        ...
        for ent in entities:
            ...
            entity_lookup[str(ent["name"]).strip().lower()] = ent
        ...
        episode = {
            "name": f"segment_{int(segment.get('index', idx)) + 1:04d}",
            "body": raw_text,
            ...
            "metadata": {
                ...
                "extraction_metadata": dict(segment.get("extraction_metadata", {})),
            },
        }
...
def determine_entity_layer(entity: dict) -> str:
    ...
    if ent_type in {"WorldRule", "Ability", "Skill", "MagicSystem", "CalendarEra"}:
        return LAYER_RULE
    if ent_type in {"HistoricalEvent", "Arc", "NarrativeArc", "TimelineEvent", "CharacterAge", "Memory", "Event"}:
        return LAYER_STORY
    if ent_type == "Concept":
...
def determine_edge_layer(edge: dict, entity_lookup: dict) -> str:
    ...
    source = _lookup_entity(entity_lookup, edge.get("source_name"))
    target = _lookup_entity(entity_lookup, edge.get("target_name"))
    ...
    if edge_type in {"Regrets", "Fears"} and LAYER_CONCEPT in {source_layer, target_layer}:
        return LAYER_CONCEPT
...
def _lookup_entity(entity_lookup: dict, entity_name: object) -> dict | None:
    raw_name = str(entity_name or "").strip()
    if not raw_name:
        return None
    return entity_lookup.get(raw_name) or entity_lookup.get(raw_name.lower())
